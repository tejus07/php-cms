...
<body>
	...
	<?php include('common/navbar.php');?>
        <section class="admin-content">
            <form class="add-new-form" action="edit-vehicle.php?id=<?php echo $data['vehicle_id']?>" method="post" enctype="multipart/form-data">
            
                <?php if(!empty($data['image_url'])) { ?>
                    <label for="uploaded_image">Uploaded Image:</label>
                    <img src="../<?php echo $data['image_url']?>" alt="<?php echo $data['image_url']?>" width="300">

                    <label for="delete_image">Delete Image: <input type="checkbox" id="delete_image" name="delete_image" value="delete"></label>
                <?php }?>
                ...
                <select id="user_id" name="user_id" value="<?php echo $data['owner_id']?>">
                    <?php foreach ($users as $userId => $username) : ?>
                        <option value="<?php echo $userId; ?>" <?php echo $userId == $data['owner_id'] ? 'selected' : ''?>><?php echo $username; ?></option>
                    <?php endforeach; ?>
                </select>
                <br><br>
                ...
                <select id="category_id" name="category_id" value="<?php echo $data['category_id']?>">
                    <?php foreach ($categories as $category_id => $category_name) : ?>
                        <option value="<?php echo $category_id; ?>" <?php echo $category_id == $data['category_id'] ? 'selected' : ''?>><?php echo $category_name; ?></option>
                    <?php endforeach; ?>
                </select>
                <br><br>
                ...
                <select id="brand_id" name="brand_id" value="<?php echo $data['brand_id']?>">
                    <?php foreach ($brands as $brand_id => $brand_name) : ?>
                        <option value="<?php echo $brand_id; ?>" <?php echo $brand_id == $data['brand_id'] ? 'selected' : ''?>><?php echo $brand_name; ?></option>
                    <?php endforeach; ?>
                </select>
                <br><br>
                ...
                <input type="text" id="rental_rate" name="rental_rate" value="<?php echo $data['rental_rate']?>" required><br><br>

                <label for="availability_status">Availability Status:</label>
                <select id="availability_status" name="availability_status" class="form-input" value="<?php echo $data['availability_status']?>" required>
